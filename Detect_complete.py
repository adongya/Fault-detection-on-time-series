...
@author: Jianye Su
"""
from Detect_NLSTM import Lstm_models,Nestlstm_models,GRU_models
import numpy as np
...
def load_data_old():
    filename_train = "data_set//Detect_Level4_train.csv"
    dataSet_train = pd.read_csv(filename_train, encoding='gbk')

    filename_test = "data_set//Detect_Level4_test.csv"
    dataSet_test = pd.read_csv(filename_test, encoding='gbk')
    ...
    # print("true_fault_data_train:",true_fault_data_train)
    # print("true_fault_data_test:",true_fault_data_test)
    ...
    # train_data1 = np.c_[train_label1,train_data1]
    # test_data1 = np.c_[test_label1,test_data1]
...
def classify(train_data, train_label):
    ...
    lgbmModel = LGBMClassifier(objective='regression',max_depth=5,num_leaves=25,learning_rate=0.007,n_estimators=1000,min_child_samples=80,
...
NLSTM_AC, LSTM_AC, GRU_AC, lgbm_AC, AdaBoost_AC, RF_AC, SVM_AC, DT_AC, MLP_AC, KNN_AC, LG_AC, NLSTM_f1, LSTM_f1, GRU_f1, lgbm_f1, AdaBoost_f1, RF_f1, SVM_f1, DT_f1, MLP_f1, KNN_f1, LG_f1 = classify(

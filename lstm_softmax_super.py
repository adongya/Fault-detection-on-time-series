...
def load_data_old():
    ...
    # true_fault_data_train = dataSet_train[:, [55, 61, 54, 56, 26, 57, 44, 51]]
    # true_fault_data_train = dataSet_train[:, 1:]
    true_fault_label_train = dataSet_train[:, 0]

    # true_fault_data_test = dataSet_test[700:800, [6,4,48,8,18,2,28,46,30,61,5,25,32,26,9,33]]
    true_fault_data_test = dataSet_test[:, [6, 4, 48, 8, 18, 2, 28, 46]]
    # true_fault_data_test = dataSet_test[:, [55, 61, 54, 56, 26, 57, 44, 51]]
    ...
    train_label1 = train_label1.reshape(2400,1)
    test_label1 = test_label1.reshape(800,1)
...
def Lstm_models():
    ...
    model = Sequential()
    model.add(LSTM(units = 64,input_shape=(num1,8),return_sequences=True,activation='relu'))
    model.add(Dropout(0.2))
    model.add(LSTM(units = 64,return_sequences=False,activation='relu'))
    model.add(Dense(8,activation='softmax'))
    model.compile(loss='sparse_categorical_crossentropy', optimizer='Nadam', metrics=['accuracy'])
    model.fit(train_data1,train_label1,batch_size=10, epochs=20,verbose=2)
...
def GRU_models():
    train_label1,train_data1,test_label1,test_data1 = load_data_old()
    model = Sequential()
    model.add(GRU(units = 64,input_shape=(num1,65),return_sequences=True,activation='relu'))
    model.add(Dropout(0.2))
...
def Nestlstm_models():
    train_label1,train_data1,test_label1,test_data1 = load_data_old()
    model1 = Sequential()
    model1.add(NestedLSTM(16, depth=2, dropout=0, recurrent_dropout=0.2,activation='relu',return_sequences=True))
    # model1.add(Dropout(0.2))
    model1.add(NestedLSTM(16, depth=2, dropout=0, recurrent_dropout=0.2,activation='relu'))

    model1.add(Dense(8,activation='softmax'))
    model1.compile(loss='sparse_categorical_crossentropy', optimizer='Nadam', metrics=['accuracy'])
    model1.fit(train_data1,train_label1,batch_size=8, epochs=20,verbose=2)
